...
static inline void Ci_cpu_pause() {
  _mm_pause();
}
...
static inline void gc_set_refs(PyGC_Head* g, Py_ssize_t refs) {
...
static inline void gc_reset_refs(PyGC_Head* g, Py_ssize_t refs) {
  g->_gc_prev = (g->_gc_prev & _PyGC_PREV_MASK_FINALIZED) |
      PREV_MASK_COLLECTING | ((uintptr_t)(refs) << _PyGC_PREV_SHIFT);
}

static inline void gc_decref(PyGC_Head* g) {
  _PyObject_ASSERT_WITH_MSG(
      FROM_GC(g), gc_get_refs(g) > 0, "refcount is too small");
...
static inline int gc_list_is_empty(PyGC_Head* list) {
...
static inline void gc_list_clear_collecting(PyGC_Head* collectable) {
  PyGC_Head* gc;
 ...
 */
static size_t update_refs(PyGC_Head* containers) {
...
static void move_unreachable(PyGC_Head* young, PyGC_Head* unreachable) {
  ...
  while (gc != young) {
    if (gc_get_refs(gc)) {
      ...
      _PyObject_ASSERT_WITH_MSG(
          op, gc_get_refs(gc) > 0, "refcount is too small");
...
static int has_legacy_finalizer(PyObject* op) {
 ...
 */
static void move_legacy_finalizers(
...
static inline void clear_unreachable_mask(PyGC_Head* unreachable) {
  ...
  for (gc = GC_NEXT(unreachable); gc != unreachable; gc = next) {
    _PyObject_ASSERT(
        (PyObject*)FROM_GC(gc), gc->_gc_next & NEXT_MASK_UNREACHABLE);
    gc->_gc_next &= ~NEXT_MASK_UNREACHABLE;
    next = (PyGC_Head*)gc->_gc_next;
  }
 ...
 * into finalizers set.
 */
static void move_legacy_finalizer_reachable(PyGC_Head* finalizers) {
  traverseproc traverse;
...
static void debug_cycle(const char* msg, PyObject* op) {
 ...
 */
static void handle_legacy_finalizers(
 ...
 */
static void finalize_garbage(PyThreadState* tstate, PyGC_Head* collectable) {
  destructor finalize;
  PyGC_Head seen;
...
static void show_stats_each_generations(GCState* gcstate) {
  char buf[100];
  size_t pos = 0;
...
2. Traverse all objects in "base" and visit all referred objects using
   ...
   reference count, while all unreachable objects must have a count of exactly
0.
3. Identify all unreachable objects (the ones with 0 reference count) and move
   them to the "unreachable" list. This step also needs to move back to "base"
all objects that were initially marked as unreachable but are referred
transitively by the reachable objects (the ones with strictly positive reference
count).
...
static inline void deduce_unreachable(PyGC_Head* base, PyGC_Head* unreachable) {
...
moving them to 'old_generation' and placing the rest on 'still_unreachable'.
...
IMPORTANT: After a call to this function, the 'still_unreachable' set will have
the PREV_MARK_COLLECTING set, but the objects in this set are going to be
removed so we can skip the expense of clearing the flag to avoid extra
iteration. */
static inline void handle_resurrected_objects(
    PyGC_Head* unreachable,
...
static void Ci_deduce_unreachable_parallel(
...
static void Ci_Sema_Init(Ci_Sema* sema) {
  sema->tokens_left = 0;
  MUTEX_INIT(sema->lock);
  COND_INIT(sema->cond);
}

static void Ci_Sema_Post(Ci_Sema* sema, size_t tokens) {
...
static void Ci_Sema_Wait(Ci_Sema* sema) {
  MUTEX_LOCK(sema->lock);
...
static void Ci_Sema_Fini(Ci_Sema* sema) {
...
static void Ci_Barrier_Init(Ci_Barrier* barrier, int capacity) {
...
static inline int Ci_gc_is_collecting_atomic(PyGC_Head* g) {
  uintptr_t prev = atomic_load_explicit(
      (atomic_uintptr_t*)(&g->_gc_prev), memory_order_relaxed);
  return (prev & PREV_MASK_COLLECTING) != 0;
}

static inline void Ci_gc_get_collecting_and_finalized_atomic(
    PyGC_Head* g,
    int* collecting,
    int* finalized) {
  uintptr_t prev = atomic_load_explicit(
...
static inline void Ci_gc_decref_atomic(PyGC_Head* g) {
  atomic_fetch_sub_explicit(
      (atomic_uintptr_t*)(&g->_gc_prev),
      1 << _PyGC_PREV_SHIFT,
      memory_order_relaxed);
}

static inline int Ci_gc_is_collecting_and_reachable_atomic(
    PyGC_Head* g,
    int* finalized) {
  uintptr_t prev = atomic_load_explicit(
      (atomic_uintptr_t*)(&g->_gc_prev), memory_order_relaxed);
...
static inline void Ci_gc_mark_reachable_and_clear_collecting_atomic(
    PyGC_Head* g,
    int finalized) {
  assert(finalized == 0 || finalized == 1);
  uintptr_t val = (1 << _PyGC_PREV_SHIFT) | (finalized);
  atomic_store_explicit(
      (atomic_uintptr_t*)(&g->_gc_prev), val, memory_order_relaxed);
}

// Subtract an incoming ref to op
static int Ci_subtract_incoming_ref(PyObject* obj, Ci_ParGCWorker* worker) {
...
static void Ci_ParGCWorker_SubtractRefs(Ci_ParGCWorker* worker) {
...
static int Ci_queue_obj_for_marking(PyObject* op, Ci_ParGCWorker* worker) {
  worker->mark_load++;
  if (!_PyObject_IS_GC(op)) {
...
static PyObject* Ci_ParGCWorker_MaybeSteal(Ci_ParGCWorker* worker) {
...
static void Ci_ParGCWorker_MarkGCSlice(Ci_ParGCWorker* worker) {
  ...
  for (PyGC_Head* gc = worker->gc_slice.start; gc != worker->gc_slice.end;
       gc = GC_NEXT(gc)) {
    int is_finalized;
    if (Ci_gc_is_collecting_and_reachable_atomic(gc, &is_finalized)) {
      ...
      Py_TYPE(obj)->tp_traverse(
          obj, (visitproc)Ci_queue_obj_for_marking, worker);
    } else {
...
//       +----------->|   steal   +-----------+
//                    +-----------+
static void Ci_ParGCWorker_ProcessMarkQueueAndSteal(Ci_ParGCWorker* worker) {
...
// GC.
static void Ci_gc_steal_backoff(int* i) {
...
static int Ci_ParGCWorker_TakeStealCoordinator(Ci_ParGCWorker* worker) {
  int success = 0;
...
static void Ci_ParGCWorker_DropStealCoordinator(Ci_ParGCWorker* worker) {
  Ci_ParGCState* par_gc = worker->par_gc;
  MUTEX_LOCK(par_gc->steal_coord_lock);
...
static void Ci_ParGCWorker_CoordinateStealing(Ci_ParGCWorker* worker) {
  ...
  while (1) {
    ...
    if (work_available) {
      ...
      if (num_workers_to_wake_up > 0) {
        CI_DLOG(
            "Waking up %lu workers, %d active, %lu inactive\n",
            num_workers_to_wake_up,
            num_workers_marking,
            num_inactive_workers);
...
static void Ci_ParGCWorker_MarkReachable(Ci_ParGCWorker* worker) {
...
static void Ci_ParGCWorker_Init(
    Ci_ParGCWorker* worker,
    Ci_ParGCState* par_gc,
    unsigned int seed) {
...
static void Ci_ParGCWorker_Fini(Ci_ParGCWorker* worker) {
  Ci_WSDeque_Fini(&worker->deque);
}

// Stolen from os_cpu_count_impl in posixmodule.c
static int Ci_get_num_processors() {
  int ncpu = 1;
#ifdef MS_WINDOWS
...
static int Ci_get_default_num_par_gc_threads() {
  int num_threads = Ci_get_num_processors() / 2;
  return num_threads > 0 ? num_threads : 1;
}

static void Ci_ParGCState_Destroy(Ci_ParGCState* par_gc);

static Ci_ParGCState* Ci_ParGCState_New(size_t min_gen, size_t num_threads) {
  if (min_gen >= NUM_GENERATIONS) {
    _PyErr_SetString(
        _PyThreadState_GET(), PyExc_ValueError, "invalid generation");
    return NULL;
  }
  if (num_threads == 0) {
  ...
  Ci_ParGCState* par_gc = (Ci_ParGCState*)PyMem_RawCalloc(
      1, sizeof(Ci_ParGCState) + sizeof(Ci_ParGCWorker) * num_threads);
  if (par_gc == NULL) {
...
static void Ci_assign_worker_slices(
    Ci_ParGCWorker* workers,
    int num_workers,
    PyGC_Head* base,
    int num_objects) {
...
static void Ci_report_load(Ci_ParGCWorker* workers, int num_workers) {
  CI_STAT(
      "%-17s  %-10s  %-13s  %-11s  %-11s  %-13s",
      "Thread ID",
      "mark load",
      "sub_refs load",
      "steal succs",
  ...
  for (int i = 0; i < num_workers; i++) {
    ...
    CI_STAT(
        "T%-16lu  %-10lu  %-13lu  %-11lu  %-11lu  %-13d",
        w->thread_id,
        w->mark_load,
        w->subtract_refs_load,
        w->steal_successes,
        w->steal_attempts,
        Ci_WSDeque_GetNumResizes(&w->deque));
  ...
  CI_STAT(
      "     steal success ratio: %lu/%lu (%.2f%%)",
      total_steals,
      total_steal_attempts,
      100.0 * total_steals / total_steal_attempts);
}

static void Ci_move_unreachable_parallel(
    PyGC_Head* base,
    PyGC_Head* unreachable) {
  // Visit all GC objects, moving anything with a refcount of 0 to unreachable,
  // and fix up prev pointers.
  PyGC_Head* prev = base;
...
static void Ci_restore_prev_ptrs(PyGC_Head* containers) {
  PyGC_Head* prev = containers;
  for (PyGC_Head* gc = GC_NEXT(containers); gc != containers;
       gc = GC_NEXT(gc)) {
...
/* Deduce which objects among "base" are unreachable from outside the list in
   ...
   2. The main GC thread wakes up each worker thread and waits for them all to
      finish.
   3. Each worker thread performs step (2) from above on its slice of the GC
list and notifies the main thread when its complete.
...
static void Ci_deduce_unreachable_parallel(
  ...
  if (num_objects < par_gc->num_workers) {
    CI_DLOG(
        "Too few objects to justify parallel collection. Collecting serially.");
  ...
  Ci_assign_worker_slices(
      par_gc->workers, par_gc->num_workers, base, num_objects);
  Ci_ParGCWorker* workers = par_gc->workers;
  for (size_t i = 0; i < par_gc->num_workers; i++) {
    workers[i].thread_id = PyThread_start_new_thread(
        (void (*)(void*))Ci_ParGCWorker_Run, &workers[i]);
  }
...
static int Ci_is_par_gc(Ci_PyGCImpl* impl) {
  return impl->collect == gc_collect_main &&
...
#else
  _PyErr_SetString(
      tstate, PyExc_RuntimeError, "not supported on this platform");
  return -1;
#endif
}

PyObject* Cinder_GetParallelGCSettings() {
  PyThreadState* tstate = _PyThreadState_GET();
